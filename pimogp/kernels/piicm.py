...
class PIICM(Kernel):
    def __init__(self, data_covar_module, num_combinations, num_cell_lines, symmetric=True, drug_rank=1,
                 ...
                 **kwargs):
        super(PIICM, self).__init__(**kwargs)
        # Check for CUDA
    ...
    def forward(self, x1, x2, diag=False, last_dim_is_batch=False, **params):
        ...
        if self.symmetric:
            ...
            covar_drugcombo_sym_row = InterpolatedLinearOperator(covar_drugcombo_t,
            ...
            covar_drugcombo_sym_total = InterpolatedLinearOperator(covar_drugcombo_tt,
                                                              left_interp_indices=self.symmetric_indices,
                                                              left_interp_values=self.symmetric_values,
            ...
            if x1.shape[1] > 1:  # For the 2-d case we flip the axis
                ...
                kron_lt1 = KroneckerProductLinearOperator(covar_x, covar_cellline,
                                                      0.25 * covar_drugcombo + 0.25 * covar_drugcombo_sym_total)
                kron_lt2 = KroneckerProductLinearOperator(covar_x_reflected, covar_cellline,
                                                      0.25 * covar_drugcombo_sym_row +
                                                      0.25 * covar_drugcombo_sym_row.t())
                res = GPattKroneckerProductLinearOperator(kron_lt1) + GPattKroneckerProductLinearOperator(kron_lt2)

            else:
                covar_k = 0.25 * covar_drugcombo + 0.25 * covar_drugcombo_sym_row + \
                          0.25 * covar_drugcombo_sym_row.t() + 0.25 * covar_drugcombo_sym_total
                kron_lt = KroneckerProductLinearOperator(covar_x, covar_k, covar_cellline)
                res = GPattKroneckerProductLinearOperator(kron_lt)
        else:
            kron_lt = KroneckerProductLinearOperator(covar_x, covar_cellline, covar_drugcombo)
            res = GPattKroneckerProductLinearOperator(kron_lt)
